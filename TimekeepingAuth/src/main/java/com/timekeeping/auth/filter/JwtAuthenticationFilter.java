package com.timekeeping.auth.filter;

import com.fasterxml.jackson.databind.ObjectMapper;
import com.timekeeping.auth.service.impl.UserDetailsServiceImpl;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            TimeKeepingCustomHttpServletRequest timekeepingRequest = new TimeKeepingCustomHttpServletRequest(request);
            ...
            for (String whiteListItem : ApiWhiteListContextPathConst.authWhiteList) {
                if (new AntPathMatcher().match(whiteListItem, servletContextPath)) {
                    if (StringUtil.isNotEmpty(request.getHeader(ApplicationConstant.AUTHENTICATION_SCHEME_NAME))) {
                        timekeepingRequest.removeHeader(ApplicationConstant.AUTHENTICATION_SCHEME_NAME);
                        hasRemoveToken = true;
                        break;
                    }
                }
            ...
            if (hasRemoveToken) {
                filterChain.doFilter(timekeepingRequest, response);
                return;
            } else {
            ...
            response.getWriter().write(
                    //                    mapper.writeValueAsString(new ResponseData<Object>(0, new ApiError("401", null, ex.getMessage())))
                    mapper.writeValueAsString(new ApiError("401", null, ex.getMessage()))
            );
            return;
        }
